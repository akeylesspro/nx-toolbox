...
export const BoardsTable = memo(({ data }: BoardsProps) => {
    ...
    const headers = ["index", t("imei"), t("sim"), t("status"), t("type"), t("comments"), t("created_date"), t("actions")];

    const keysToRender = ["index", "imei", "sim", "ui_status", "type", "comments", "ui_uploaded", "actions"];

    const sortKeys = ["index", "imei", "sim", "status", "type", "comments", "created_date", "actions"];
    ...
    return (
        <div style={{ direction: direction }} className="w-full h-full _center">
            {formattedData.length ? (
                <Table
                    headerStyle={{ backgroundColor: "#0f172a", height: "30px" }}
                    containerHeaderClassName="h-12"
                    containerClassName="_full"
                    cellClassName="_ellipsis text-start h-8"
