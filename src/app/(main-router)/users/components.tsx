  ...
  const formattedData = useMemo(() => {
    return data.map((user) => {
      ...
      return {
        ...
        last_login_ui,
        updateUserLastLogin: user.last_login ? timestamp_to_string(user.last_login as any as Timestamp) : "",
          ...
          .map(([entity, featureArray], index, array) => {
            ...
            return (
              <div key={entity} className={cn(lineClassNames, `border-[#5f9ea0] px-1.5`)}>
                <CheckBoxGroup entity={entity} features={featureArray} onChecked={onChecked} selectedFeatures={selectedFeatures} user={user} />
              </div>
            );
          })}
      </div>
